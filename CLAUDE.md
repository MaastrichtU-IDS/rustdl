...
> **FOUR OF THE "MISSING" FIXTURES ARE RECONSTRUCTIBLE LOCALLY, AND DOING SO TOOK THE NET
> FROM 15/7 TO 20/2 (2026-08-25).** `scripts/fetch-real-ontologies.sh` provisions *inputs*
> only and never oracles, so the standing advice "needs the fixtures, run the fetch script"
> is wrong for the cases that actually fail — `pizza.ofn` was already present and it was the
...
>
> **Validate the pipeline before trusting a generated oracle.** The `.kon.owx` artifacts are
> NOT byte-identical to the committed `-classified.owx` files, so the control is to swap one
> in for a fixture whose answer is known: `eval-tools/work/sio.kon.owx` reproduces
> `sio 8904=8904 FP=0 MISSED=0` exactly, proving the byte differences are serialization, not
> content.
>
> **The result independently re-derives four of the reference values above** — pizza 499,
> bibtex 16, ore-10908 6001, ore-15672 142, all FP=0/MISSED=0 — from oracles generated by a
> *different* reasoner binary than the originals, **including the two `corpus.md` attributes
> to HermiT**. So Konclude and HermiT agree on those closures and the ORE tests' `_matches_hermit`
> names are not a provenance hazard.
>
> **`galen` is the one that does NOT reproduce: 28007, not the 27997 recorded above.** The
> *oracle* reads 28007 too and its file predates this by six weeks, so it is fixture-vintage
> drift in this document, not a reasoner change — and the four exact matches are what make
> that attribution safe. Treat 27997 as stale.
>
> **`alehif-test` and `notgalen` remain unobtainable** — no provenance is recorded anywhere in
> `docs/`, and neither exists on disk. Konclude's own `Tests/galen-ALEHIF+-classify-request.xml`
> is NOT the source: it is a 3.1 MB inline-OWLlink galen variant, against `alehif-test`'s 167
> classes / 247-closure. Those two are the whole of the net's residual 2 failures.
...
- **`crates/owl-dl-core`** — Phase 3c (commit 0b5ed36) cached
  ...
  **EVIDENCE, with its limits stated.** The pass is **INERT on all 7 curated fixtures**
  ...
  FP=0 net after provisioning four previously-absent fixtures (see the soundness-net block
  above): **20 passed / 2 failed**, 11 fixtures VERIFIED with every closure exact —
