...
Prefix(owl:=<http://www.w3.org/2002/07/owl#>)\n";
    ...
    /// STAGE-4 revisit probe (ignored): run sat(Gamay) [genuinely-hard] with
    /// RUSTDL_REVISIT_PROBE on — the wedge prints visits vs distinct node-states.
    /// distinct ≪ visits ⇒ re-computation of a tiny state set ⇒ memoization is
    /// the lever (post-∃-seed); distinct ≈ visits ⇒ genuinely huge space.
    #[test]
    #[ignore = "Stage-4 diagnostic; --ignored --nocapture"]
    fn stage4_revisit_gamay() {
        use horned_owl::io::ofn::reader::read as read_ofn;
        // SAFETY: single ignored test; sets its own diagnostic env var.
        unsafe { std::env::set_var("RUSTDL_REVISIT_PROBE", "1") };
