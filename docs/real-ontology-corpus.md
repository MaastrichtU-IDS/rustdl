...
| `wine` | W3C OWL-guide `wine` + `food` (`http://www.w3.org/TR/2003/PR-owl-guide-20031209/{wine,food}`), merged | RDF/XML | The canonical SHOIN(D) tutorial ontology — **nominal-heavy** (207 `ObjectHasValue`/`ObjectOneOf` value restrictions on regions/grapes/colors) + 39 `DisjointClasses` + 88 equivalences in 137 classes. The corpus's stressor for **nominal / value-restriction reasoning**, a class of entailment no other fixture exercised. wine imports food which circularly imports wine; `fetch_wine` strips both `owl:imports` and merges locally. **FP=0, MISSED 57 → 34 → 31 → 29 → 9 → 8 → 2.** 57→34: nominal lever (region/color cluster via NomKey fold + transitive-ABox propagation, `docs/nominal-lever-scoping-2026-06-06.md`). 34→31: precise `≤n`-cardinality clash deps unblock wedge backjumping (v0.3.5, `docs/backjump-reconcile-2026-06-06.md`). 31→29: defined-SUB sweep (`food#Fruit`, `docs/classify-recovery-scope-2026-06-07.md`). 29→9: MaxKey lever (defined-class `≤n`-cardinality conjunct, appellation⊑varietal `Beaujolais ⊑ Gamay`, +20 cascade). 9→2: ForallKey lever (defined-class `∀R.OneOf` conjunct, two seeds: told-`∀` + functional+`∃`nominal→`∀`), recovering the sugar `⊑WhiteNonSweetWine` cluster. **Residual 2** = both `Sancerre` (⊑SauvignonBlanc + ⊑WhiteNonSweetWine) — the one appellation that never acquires WhiteWine; the last nominal frontier (`docs/wine-residual-31-diagnosis-2026-06-07.md`). See `docs/corpus-wine-2026-06-06.md`. |
