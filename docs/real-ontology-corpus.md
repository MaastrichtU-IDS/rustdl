...
| `wine` | W3C OWL-guide `wine` + `food` (`http://www.w3.org/TR/2003/PR-owl-guide-20031209/{wine,food}`), merged | RDF/XML | The canonical SHOIN(D) tutorial ontology — **nominal-heavy** (207 `ObjectHasValue`/`ObjectOneOf` value restrictions on regions/grapes/colors) + 39 `DisjointClasses` + 88 equivalences in 137 classes. The corpus's stressor for **nominal / value-restriction reasoning**, a class of entailment no other fixture exercised. wine imports food which circularly imports wine; `fetch_wine` strips both `owl:imports` and merges locally. **FP=0, MISSED 57 → 34 → 31 → 29.** 57→34: nominal lever (region/color cluster via NomKey fold + transitive-ABox propagation, `docs/nominal-lever-scoping-2026-06-06.md`). 34→31: precise `≤n`-cardinality clash deps unblock wedge backjumping (v0.3.5, `docs/backjump-reconcile-2026-06-06.md`). 31→29: defined-SUB sweep recovers covering-defined ⊑ primitive-sup (`food#Fruit ⊑ EdibleThing/ConsumableThing`, `docs/classify-recovery-scope-2026-06-07.md`). **Residual 29** = grape (`≤1` cardinality) + sugar (`∀`+nominal set) + region→grape varietal — the nominal/functional/cardinality modeling frontier (`docs/wine-residual-31-diagnosis-2026-06-07.md`), confirmed not provable by the tableau even unbounded. See `docs/corpus-wine-2026-06-06.md`. |
