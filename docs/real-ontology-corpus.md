...
| `wine` | W3C OWL-guide `wine` + `food` (`http://www.w3.org/TR/2003/PR-owl-guide-20031209/{wine,food}`), merged | RDF/XML | The canonical SHOIN(D) tutorial ontology — **nominal-heavy** (207 `ObjectHasValue`/`ObjectOneOf` value restrictions on regions/grapes/colors) + 39 `DisjointClasses` + 88 equivalences in 137 classes. The corpus's stressor for **nominal / value-restriction reasoning**, a class of entailment no other fixture exercised. wine imports food which circularly imports wine; `fetch_wine` strips both `owl:imports` and merges locally. **FP=0, MISSED 57 → 34 → 31 → 29 → 9 → 8 → 2 → 0 (full Konclude parity, 653=653).** 57→34: nominal lever (region/color cluster via NomKey fold + transitive-ABox propagation, `docs/nominal-lever-scoping-2026-06-06.md`). 34→31: precise `≤n`-cardinality clash deps unblock wedge backjumping (v0.3.5, `docs/backjump-reconcile-2026-06-06.md`). 31→29: defined-SUB sweep (`food#Fruit`, `docs/classify-recovery-scope-2026-06-07.md`). 29→9: MaxKey lever (defined-class `≤n`-cardinality conjunct, appellation⊑varietal `Beaujolais ⊑ Gamay`, +20 cascade). 9→2: ForallKey lever (defined-class `∀R.OneOf` conjunct; seeds: told-`∀` + functional+`∃`nominal→`∀`), sugar `⊑WhiteNonSweetWine`. 2→0: ForallKey ≤1-driven variant (per-class `≤1 R` + `∃R.{a}`→`∀R.OneOf`, R need not be globally functional), closing the Sancerre/SemillonOrSauvignonBlanc chain. **Residual 0 — full Konclude parity** (FP=0 AND MISSED=0). The nominal-completeness project is complete; see `docs/wine-residual-31-diagnosis-2026-06-07.md` for the four levers (defined-SUB sweep, MaxKey, ForallKey ×2 paths + ≤1-driven). See `docs/corpus-wine-2026-06-06.md`. |

| `bibtex` | ORE-2015 sample (`ore_ont_3341.owl` = `edu.mit.visus.bibtex`) | RDF/XML | **Datatype-heavy** real fixture: 41 `DataMinCardinality` + 40 `DataPropertyDomain` + 39 `DataPropertyRange`, 15 classes, 16-pair closure. Validates Phase-D on real data — `fetch_bibtex` extracts + converts from `ore2015_sample.zip`. **FP=0, MISSED=0** (full HermiT parity). Meta-finding: real datatypes don't drive *non-redundant* classification (hierarchies are told), so this is the real-data soundness guard while the synthetic `tests/fixtures/datatype/` suite remains the D4/D5 completeness test. See `docs/corpus-datatype-2026-06-06.md`. |
